...
export class AuthService {
  ...
  constructor(
    ...
    datalocal.getUserConfig().then(conf => {
    ...
    setTimeout(() => {
      //console.log(this.userConfig['user'])
      //this.logout()
      //this.loginToken(this.userConfig['user'].stsTokenManager.accessToken)
    }, 8000);
  ...
  async getUserLoging() {
    ...
    this.auth.onAuthStateChanged((user: any) => {
      if (user) {
        ...
        this.userLogin = {... user};
        this.userLogin.fechalogin = new Date().toISOString();
        this.userLogin.password = this.uncryptPassword(this.userLocal['password'])
        //console.log("**getUser ", "..NO USER", user, this.userLogin)
        //this.login(user)
      }
      //x = user
      //console.log(user)
  ...
  setUserLoging() {
    ...
    this.datalocal.setConfig("user", usuario)
    this.datalocal.setConfig("role", usuario.role)
    setTimeout(() => {
    }, 1000);
